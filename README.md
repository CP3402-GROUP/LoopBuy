...
- Scam screening with a versioned TF-IDF/logistic-regression baseline plus explicit lexical risk signals. It is deploy-time switchable: the current Compose default temporarily publishes listings as explicitly `not_screened` (never Verified), while recommendation reranking remains enabled; turning moderation back on restores bounded low-risk publication and distinct review/unavailable states.
